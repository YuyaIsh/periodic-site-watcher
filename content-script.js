 ...
 * @param {Object} [collectContext] - Service Worker から渡す実行時コンテキスト（x-bookmarks パイプライン等）
 * @returns {Promise<Object>} 最小契約に準拠したpayload
 * @throws {Error} 未対応のsiteIdの場合
 */
async function collectOnPage(siteId, site = {}, collectContext = {}) {
  ...
  const proxy = new Proxy(origConsole, {
    get(target, prop, receiver) {
      ...
      if (methods.includes(name) && typeof Reflect.get(target, prop, receiver) === 'function') {
      ...
      (async () => {
        try {
          ...
          const collectContext =
            ...
            typeof message.collectContext === 'object'
              ? message.collectContext
              : {};
          const result = await collectOnPage(message.siteId, site, collectContext);
